...
select t.team_id,
t.team_name,
count(p.player_id) as total_number_of_players
from teams t
join players p
...
select t.team_name,
count(m.winner_id) as number_of_matches_won
from teams t
join matches m
...
select t.team_name,
count(m.winner_id) as most_matches_won
from teams t 
join matches m
on t.team_id = m.winner_id
group by t.team_name
...
select t.team_id,
 t.team_name,
count(p.player_id) as num_of_players
from teams t
join players p
on t.team_id = p.team_id
where salary > 100000
