...
INSERT INTO Customer (CustomerID, FirstName, LastName, City, State)
...
select count(accountID) as num_accounts
from accounts;
...
select b.branchname,avg(a.balance) as high_avg_balnce
from branches b
join accounts a
...
select a.customerid,concat(c.firstname,' ' ,Lastname),sum(a.balance) as total_balance
from accounts a
join customer c
on a.customerid = c.customerid
group by customerid 
...
select concat(c.firstname,' ',lastname),count(t.transactionid) as total_transaction
from customer c
join transactions t
...
select a.customerid,sum(a.balance) as total_balance,c.firstname
from accounts a
join customer c
on a.customerid = c.customerid
where accounttype in ( "saving" and "checking" )
...
select b.branchname,count(t.transactionid) as total_transactions
from branches b
join accounts a 
on b.branchid = a.branchid
join transactions t
